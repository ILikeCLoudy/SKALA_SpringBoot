...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<String> handleInternalServerException(RuntimeException ex) {
        return ResponseEntity.internalServerError().body("Global 400 Error: " + ex.getMessage());
    }

    // 기타 모든 예외 처리 (예상하지 못한 모든 예외 - 추후 추가 가능)
    @ExceptionHandler(Exception.class)
    public ResponseEntity<String> handleException(Exception ex) {
        return ResponseEntity.internalServerError().body("Global Error: 알 수 없는 오류가 발생하였습니다. 관리자에게 문의하세요." );
    }
}
